...
public class NamespaceGenerator : BaseNativeGenerator<Namespace>
{
  ...
  /// <remarks>
  /// Note: kubectl create namespace supports basic namespace creation with name.
  /// Additional metadata like labels and annotations are not supported by the kubectl create command
  /// and will be ignored.
  /// </remarks>
  static ReadOnlyCollection<string> AddArguments(Namespace model)
  {
    ...
    // Add field-manager if specified
    if (!string.IsNullOrEmpty(model.FieldManager))
    {
      args.Add($"--field-manager={model.FieldManager}");
    }
