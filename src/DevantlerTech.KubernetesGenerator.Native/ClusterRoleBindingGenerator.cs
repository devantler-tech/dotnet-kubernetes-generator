...
public class ClusterRoleBindingGenerator : BaseNativeGenerator<ClusterRoleBinding>
{
  ...
  static ReadOnlyCollection<string> AddArguments(ClusterRoleBinding model)
  {
    ...
    // Validate RoleRef kind - ClusterRoleBinding can only reference ClusterRoles
    if (model.RoleRef.Kind != RoleBindingRoleRefKind.ClusterRole)
      throw new KubernetesGeneratorException($"ClusterRoleBinding can only reference ClusterRoles, but got '{model.RoleRef.Kind}'.");

    // Add cluster role reference
